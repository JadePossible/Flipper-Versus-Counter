...
static void render_callback(Canvas* canvas, void* ctx) {
    ...
    canvas_draw_str_aligned(canvas, 64, 10, AlignCenter, AlignCenter, "Player 1       Player 2");
    canvas_set_font(canvas, FontBigNumbers);

    char scount[5];
    uint16_t dynamicBoxWidth = BOXWIDTH;
    uint16_t dynamicBoxWidth2 = BOXWIDTH;
    uint16_t dynamicMiddleX = MIDDLE_X-32;
    uint16_t dynamicMiddleX2 = MIDDLE_X+32;

    if(c->count > 99) {
        dynamicBoxWidth = BOXWIDTH_BIG;
        dynamicMiddleX = MIDDLE_X_BIG-32;
    }
    if(c->count2 > 99) {
        dynamicBoxWidth2 = BOXWIDTH_BIG;
        dynamicMiddleX2 = MIDDLE_X_BIG+32;
    }

    if(c->pressed == true || c->boxtimer > 0) {
        canvas_draw_rframe(canvas, dynamicMiddleX2, MIDDLE_Y + OFFSET_Y, dynamicBoxWidth2, BOXHEIGHT, 5);
        ...
        canvas_draw_rframe(canvas, dynamicMiddleX2, MIDDLE_Y + OFFSET_Y, dynamicBoxWidth2, BOXHEIGHT, 5);
        canvas_draw_rframe(
            canvas, dynamicMiddleX2 - 1, MIDDLE_Y + OFFSET_Y - 1, dynamicBoxWidth2 + 2, BOXHEIGHT + 2, 5);
        canvas_draw_rframe(
            canvas, dynamicMiddleX2 - 2, MIDDLE_Y + OFFSET_Y - 2, dynamicBoxWidth2 + 4, BOXHEIGHT + 4, 5);
        c->pressed2 = false;
        c->boxtimer2--;
    } else {
        canvas_draw_rframe(canvas, dynamicMiddleX, MIDDLE_Y + OFFSET_Y, dynamicBoxWidth, BOXHEIGHT, 5);
    ...
    canvas_draw_str_aligned(canvas, 32, 32 + OFFSET_Y, AlignCenter, AlignCenter, scount);
    furi_mutex_release(c->mutex);



    snprintf(scount, sizeof(scount), "%d", c->count2);
    canvas_draw_str_aligned(canvas, 96, 32 + OFFSET_Y, AlignCenter, AlignCenter, scount);
    furi_mutex_release(c->mutex);
}
